...
|         list lobbies        |   POST  | `/lobby/list_lobbies/`   | `{PRIVATE}` `WantedLobbies{ WantedLobbies_from: Optional[int], WantedLobbies_end_at: Optional[int]}` | 200 - OK `LobbyDict{ lobbyDict: Dict}` \ 400 - Bad Request: `WantedLobbies_from` value must be bigger than `WantedLobbies_end_at` value | POST: Dict format = {"lobby_name" : lobby_id}; If both optional values are left empty, list_lobbies will show all the currently existing lobbies |
...
|          change nick        |   POST  | `/lobby/{lobby_id}/change_nick`     | `{PRIVATE} {nick: str}`       | 202 - ACCEPTED `ResponseText{ responseText: str}` \ 412 - Precondition Failed | PRE: The inserted nick is not taken; the user is in the lobby; the lobby exist; Player has enough nick points |
|          leave lobby        |  DELETE | `/lobby/<lobby_id>`                 | `{PRIVATE}`                   | 202 - ACCEPTED `ResponseText{ responseText: str}` \ 409 - Conflict \ 200 - OK | PRE: Player is in the lobby; Lobby must exist |
|          start game         |  DELETE | `/lobby/<lobby_id>/start_game`      | `{PRIVATE}`                   | 200 - Ok `ResponseText{ responseText: str}` \ 401 - Unauthorized | PRE: The ammount of players should be at least more than the pre-stablished minimum value and less than the pre-stablished maximum value |
|          list games         |  GET | `/games/list_games/`      | `{PRIVATE}` `{ from: int, end_at: int }` | 200 - OK `GameDict{ gameDict: Dict }` \ 400 - Bad Request: `from` value must be bigger than `end_at` value | POST: Dict format = {"game_id" : {"total players": int, "nick": string, "role": int, "is_alive": bool}}; If both optional values are left empty, list_games will show all the currently existing games |
...
|       vote       |   POST  | `/games/<game_id>/select_director/vote` | `{PRIVATE} Vote {vote: bool}` | 200 - `VoteOut {voteOut: bool,voteOut_game_id: int, voteOut_response: str}` \ 412 - Precondition Failed | PRE: Player is in the game; Player is not dead; Player has not voted |
|       discard card       |   POST  | `/games/{game_id}/discard_card/` | `{PRIVATE} Card{card_discarted: int}` | 200 - `Card { card_discarted: int}` \ 412 - Precondition Failed | PRE: Player is the Minister or the Director in the game; If the player is the Minister selects a card beween 1 and 3; If the player is the Director selects a card between 1 and 2 |
